...
class ControllerModuleTMFilter extends Controller {
	...
	private function _init( $tab ) {
		...
		if( ! $curr_ver || version_compare( $curr_ver, $this->_version, '<' )) {			
			...
			$add_routes = array(
				'TM Filter' => 'module/tm_filter/results',
		...
		} else if( ! file_exists( DIR_SYSTEM . '../catalog/view/theme/default/template/module/tm_filter.tpl' ) ) {
		...
		if( is_readable( __FILE__ ) ) {
			$files = glob( DIR_SYSTEM . '../catalog/view/theme/*/template/module/tm_filter.tpl' );
			$source = filemtime( DIR_SYSTEM . '../catalog/view/theme/default/template/module/tm_filter.tpl' );
			...
			foreach( $files as $id => $file ) {
				...
				if( $theme == 'default' || ! is_readable( $file ) ) {
					...
					if( $source - $time > 60 * 10 ) {
						$files[$id] = '<span style="margin-left:15px; display: inline-block;"> - /catalog/view/theme/<b>' . $theme . '</b>/template/module/tm_filter.tpl</span>';
					} else {
						unset( $files[$id] );
					}
				}
	...
	public function index() {
		...
		$this->response->setOutput( $this->load->view('module/tm_filter.tpl', $this->data) );
	}
	...
	public function ldv() {
		...
		$pagination->url = $this->url->link('module/tm_filter/ldv', 'token=' . $this->session->data['token'] . '&page={page}', 'SSL');
			
		$this->data['pagination'] = $pagination->render();
		
		$this->response->setOutput( $this->load->view('module/tm_filter_ldv.tpl', $this->data) );
	}
	...
	public function attributes() {
		...
		$pagination->limit = $limit;
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('module/tm_filter/attributes', 'token=' . $this->session->data['token'] . '&page={page}', 'SSL');
			
		$this->data['pagination'] = $pagination->render();
		
		$this->response->setOutput( $this->load->view('module/tm_filter_attributes.tpl', $this->data) );
	}
	...
	private function _sortOptions( & $options, $sort, $attribute_id, $lang_id ) {		
		...
		self::$_tmp_sort_parameters = array(
			...
			'a'				=> false,
			'options'		=> $options,
			'config'		=> $this->config->get('tm_filter_at_sort_' . $attribute_id . '_' . $lang_id )
		);
		...
		if( ! self::$_tmp_sort_parameters['type'] && self::$_tmp_sort_parameters['attribute_id'] !== NULL && self::$_tmp_sort_parameters['config'] ) {
			uasort( $options, array( 'ControllerModuleTMFilter', '_sortItems' ) );
		} else {
			$tmp = array();
	...
	private function _getOptionItems( $start = NULL, $limit = NULL, $filter = NULL ) {
		...
		$sql = "
			...
			WHERE 
				...
				`o`.`type` IN( 'select', 'radio', 'checkbox', 'image', 'color' )
		";
	...
	public function options() {
		...
		$pagination->limit = $limit;
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('module/tm_filter/options', 'token=' . $this->session->data['token'] . '&page={page}', 'SSL');
			
		$this->data['pagination'] = $pagination->render();
		
		$this->response->setOutput( $this->load->view('module/tm_filter_options.tpl', $this->data) );
	}
	...
	public function filters() {
		...
		$pagination->limit = $limit;
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('module/tm_filter/filters', 'token=' . $this->session->data['token'] . '&page={page}', 'SSL');
			
		$this->data['pagination'] = $pagination->render();
		
		$this->response->setOutput( $this->load->view('module/tm_filter_filters.tpl', $this->data) );
	}
	...
	public function settings() {
		...
		if( $this->request->server['REQUEST_METHOD'] == 'POST' && $this->checkPermission() ) {
			...
			if( $this->_hasMFilterPlus() ) {
				...
				$after = empty( $this->request->post['tm_filter_settings']['attribute_separator'] ) ? '' : $this->request->post['tm_filter_settings']['attribute_separator'];
		...
		$this->response->setOutput( $this->load->view('module/tm_filter_settings.tpl', $this->data) );
	}
	...
	public function clearcache() {
		...
		$this->response->redirect($this->url->link('module/tm_filter/settings', 'token=' . $this->session->data['token'], 'SSL'));
	}
	...
	public function install() {		
		...
		foreach( $this->model_localisation_language->getLanguages() as $language ) {
			$titles[$language['language_id']] = 'Tm Filter';
		}
		...
		$idx = 0;
		foreach( $this->db->query( "SELECT * FROM " . DB_PREFIX . "extension WHERE code='tm_filter' AND type='module'")->rows as $row ) {
			if( $idx ) {
	...
	public function uninstall() {
		...
		$this->db->query("
			DELETE FROM
				`" . DB_PREFIX . "setting`
			WHERE
				`key` IN('tm_filter_module','tm_filter_status','mfilter_version','tm_filter_attribs','tm_filter_settings','tm_filter_filters','tm_filter_options','mfilter_plus_version','mfilter_mijoshop') OR
				`key` REGEXP '^tm_filter_at_img_[0-9]+_[0-9]+$' OR 
				`key` REGEXP '^tm_filter_at_sort_[0-9]+_[0-9]+$'
		");
