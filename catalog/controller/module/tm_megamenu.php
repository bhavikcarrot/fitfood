...
class ControllerModuleTMMegaMenu extends Controller
{
	...
	public function index($setting)
	{
		...
		if (isset($setting['menu_item'])) {
			$j = 0;
			//echo '<pre>';
			//print_r($setting['menu_item']);
			usort($setting['menu_item'], 'sortByOrder');
			//print_r($setting['menu_item']);
			//exit;
								...
								switch ($menu_item['column'][$i]['content']) {
									...
									if (isset($results)) {
										foreach ($results as $product_info) {
											if($category_id==115)
											$list2 .= "<li>\n<a href=\"" . $this->url->link('product/7days', '&product_id=' . $product_info['product_id']) . "\">" . $product_info['name'] . "</a>\n</li>\n";
											else
									...
									for ($subwithsub_count = 0; $subwithsub_count < 20; $subwithsub_count++) {
												...
												if (isset($subcategories2)) {
													for ($l = 0; $l < $menu_item['column'][$i]['limit']; $l++) {
														if (isset($subcategories2[$l])) {
															...
															$subcategory2_href = $this->url->link('product/category', 'path=' . $subcategories2[$l]['category_id']);
															$list .= "<li>\n<a href=\"" . $subcategory2_href . "\">" . $subcategory_name . "</a>\n</li>\n";
														}
													}
												}
												$list1 .= "<li>\n<a href=\"" . $subcategory_href . "\">" . $subcategory['name'] . "</a><ul>" . $list . "</ul>\n</li>\n";
										}
									}
									break;
		...
		return $this->load->view('module/tm_megamenu', $data);
	}
