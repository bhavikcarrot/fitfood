...
//console.log($('.date').val());
	...
	$('.date').on('change',function(){
		$("#weekdayPlanner").remove();
			var str = '<div id="weekdayPlanner"><ul class="nav nav-tabs">';
			var tab_content = '<div class="tab-content">';
			//var str = '<table width="100%" cellspacing="0" cellpadding="0" id="weekdayPlanner"><tr>';
			...
			for(var i=1;i<=7;i++){
				...
				var act_clas='';
				str += '<li class="'+act_clas+'"><a href="#tab-days'+x+'" data-toggle="tab">'+days[x]+'</a></li>';
				var faceboxStr='<a href="index.php?route=product/7days/getProduct&product_id=51&mealType='+mealType+'&mealDay='+i+'" rel="facebox">Custom Meal</a>';
				for(var c=0;c<catArr.length;c++){
					faceboxStr = faceboxStr + '<br><a href="index.php?route=product/7days/getCategory&category='+catArr[c]+'&mealType='+mealType+'&mealDay='+i+'" rel="facebox">'+catNameArr[c]+'</a>';
				}
				tab_content += '<div id="tab-days'+x+'" class="tab-pane '+act_clas+'"><span>Create your combination of meals from below categories for '+days[x]+'</span><br>'+faceboxStr+'</div>';
			}
			/*var newtabcontent;
			//newtabcontent= 
			tab_content.find('href').each(function(){
				console.log('sssss')
			});*/
			//str += '</tr>';
			//str += '</table>';
			str += '</ul>'+tab_content;
			/*str += '<div id="tab-days1" class="tab-pane active">aaa</div>';
			str += '<div id="tab-days2" class="tab-pane">bbb</div>';
			str += '<div id="tab-days3" class="tab-pane">ccc</div>';*/
			str += '</div></div>';
			$("#input-option42").parent('div').after(str);
	});
	$(".date").trigger('change');
