...
class Cart {
	...
	public function getProducts() {
		$product_data = array();
//echo "SELECT * FROM " . DB_PREFIX . "cart WHERE customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "'";
		$cart_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "cart WHERE customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' ORDER BY daymeals");
		...
		foreach ($cart_query->rows as $cart) {
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				$option_data = array();
				$daymeals=array();
				if(isset($cart['daymeals']) && $cart['daymeals']!=''){
					foreach (json_decode($cart['daymeals']) as $daymealsId=>$daymealsVal) {
						//echo '<br>'.$daymealsId.'=>'.$daymealsVal;
						$daymeals[] = array('name'=>$daymealsId,'value'=>$daymealsVal);
					}
				}
	...
	public function add($product_id, $quantity = 1, $option = array(), $recurring_id = 0,$daymeals='') {
		
		if(is_array($daymeals)){
			//$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "cart WHERE customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' AND product_id = '" . (int)$product_id . "' AND recurring_id = '" . (int)$recurring_id . "' AND `option` = '" . $this->db->escape(json_encode($option)) . "' AND `daymeals` = '" . $this->db->escape(json_encode($daymeals)) . "'");
			$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "cart WHERE customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' AND `daymeals` = '" . $this->db->escape(json_encode($daymeals)) . "'");
			
			if (!$query->row['total']) {
				$this->db->query("INSERT " . DB_PREFIX . "cart SET customer_id = '" . (int)$this->customer->getId() . "', session_id = '" . $this->db->escape($this->session->getId()) . "', product_id = '" . (int)$product_id . "', recurring_id = '" . (int)$recurring_id . "', `option` = '" . $this->db->escape(json_encode($option)) . "', quantity = '" . (int)$quantity . "', date_added = NOW(),daymeals= '" . $this->db->escape(json_encode($daymeals)) . "'");
			} else {
				return 'exist';
				//$this->db->query("UPDATE " . DB_PREFIX . "cart SET quantity = (quantity + " . (int)$quantity . ") WHERE customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' AND product_id = '" . (int)$product_id . "' AND recurring_id = '" . (int)$recurring_id . "' AND `option` = '" . $this->db->escape(json_encode($option)) . "' AND daymeals= '" . $this->db->escape(json_encode($daymeals)) . "'");
			}	
		}
		else{
